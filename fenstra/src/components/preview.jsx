...
// Map of old colour name strings to hex codes, for backward compatibility
const COLOUR_NAME_TO_HEX = {
  "White": "#FFFFFF",
  "Anthracite Grey": "#383E42",
  "RAL 7016 Anthracite Grey": "#383E42",
  "Agate Grey": "#B5B3AB",
  "RAL 7038 Grey": "#B5B3AB",
  "RAL 7038": "#B5B3AB",
  "Grey": "#B5B3AB",
  "Black": "#1C1C1C",
  "RAL 9005 Black": "#1C1C1C",
  "RAL 9005": "#1C1C1C",
  "RAL 9016 White": "#FFFFFF",
  "RAL 9016": "#FFFFFF",
  "Black-Brown": "#3D2B1F",
  "Cream": "#F0E8D4",
  "Chartwell Green": "#8BA084",
  "Golden Oak": "#C08B3D",
  "Irish Oak": "#8B5A2B",
  "Rosewood": "#4A1F1F",
  "RAL 8017 Brown": "#4A1F1F",
  "RAL 8017": "#4A1F1F",
  "Brown": "#4A1F1F",
  "Whitegrain": "#F5F2EA",
  "Oak": "#C08B3D",
...
function resolveColour(value) {
  // Handles: null, undefined, hex strings, full names, RAL names
  if (!value) return "#FFFFFF";
  if (typeof value !== "string") return "#FFFFFF";
  const trimmed = value.trim();
  if (trimmed.startsWith("#")) return trimmed;
  // Try exact match first
  if (COLOUR_NAME_TO_HEX[trimmed]) return COLOUR_NAME_TO_HEX[trimmed];
  // Try case-insensitive lookup
  const lowerKey = Object.keys(COLOUR_NAME_TO_HEX).find(k => k.toLowerCase() === trimmed.toLowerCase());
  if (lowerKey) return COLOUR_NAME_TO_HEX[lowerKey];
  // Try partial match (e.g. "RAL 7016 Anthracite Grey (uPVC)")
  const partialKey = Object.keys(COLOUR_NAME_TO_HEX).find(k => trimmed.toLowerCase().includes(k.toLowerCase()));
  if (partialKey) return COLOUR_NAME_TO_HEX[partialKey];
  // Fallback to white
  return "#FFFFFF";
}
...
export function WindowPreview({ cfg, small = false, technical = false }) {
  ...
  // Always resolve colour to a valid hex — handles old string names
  const colour = resolveColour(rawColour);

  // Guard against invalid dimensions
  const safeWidth = Math.max(100, parseInt(width) || 1200);
  const safeHeight = Math.max(100, parseInt(height) || 1200);
  ...
  // Colour handling
  const frameColour = colour; // colour is already resolved to hex
  const woodColourNames = ["Oak", "Rosewood", "Walnut", "Golden Oak", "Irish Oak", "Whitegrain"];
  const colourLabel = (colourName || rawColour || "").toString();
  const isWood = woodColourNames.some(w => colourLabel.includes(w)) ||
                 ["Timber", "Hardwood", "Softwood"].includes(material);
